...
_.extend(WebFS.prototype,{
    ...
    parseRealPath : function(path) {
      return path.join(this._mappedRealFolder,path);
    },

    exists: function(args, resolve, reject) {
      const realPath = this.parseRealPath(args.path);
      _fs.exists(resolved.real, (exists) => {
        resolve(exists);
      });
    },

    read: function(args, resolve, reject) {
      /*eslint new-cap: "off"*/
      const resolved = this.parseRealPath(args.path);
      const options = args.options || {};
      const mime = getMime(args.path);
    ...
    upload: function(args, resolve, reject) {
    ...
    write: function(args, resolve, reject) {
      const resolved = this.parseRealPath(args.path);
      const options = args.options || {};
      let data = args.data || '';
    ...
    unlink: function(args, resolve, reject) {
      const resolved = this.parseRealPath(args.path);
      if ( ['', '.', '/'].indexOf() !== -1 ) {
    ...
    copy: function(args, resolve, reject) {
      const sresolved = this.parseRealPath(args.src);
      const dresolved = this.parseRealPath(args.dest);
    ...
    move: function(args, resolve, reject) {
      const sresolved = this.parseRealPath(args.src);
      const dresolved = this.parseRealPath((args.dest);
    ...
    mkdir: function(args, resolve, reject) {
      const resolved = this.parseRealPath(args.path);
    ...
    find: function(args, resolve, reject) {
